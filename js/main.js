...
; (function ($) {
    ...
    $(window).on('scroll', function () {
        ...
        $('section').each(function () {
            ...
            const headerHeight = 100; // Adjust this value according to your header height

            if (scrollPosition >= sectionOffset - headerHeight) {
                // Remove the active class from all menu items
                $('#menu-primary-menu .menu-item').removeClass('current-menu-item');

                // Add the active class to the corresponding menu item
                $(`#menu-primary-menu .menu-item a[href="#${sectionId}"]`).parent().addClass('current-menu-item');
            }
        });
    });

    const scrollToSection = (sectionId) => {
        const headerHeight = $('.header-fix-space').height(); // Adjust this value according to your header height
    ...
    $('.page-target').on('click', function (event) {
